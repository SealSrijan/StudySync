...
const EntryForm: React.FC<EntryFormProps> = ({ onAddEntry, streak }) => {
  ...
  // 🌙 AUTO-DATE EFFECT — updates automatically at midnight
  useEffect(() => {
    const checkDate = () => {
    ...
    const interval = setInterval(checkDate, 60 * 1000); // check every 1 min
  ...
  const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    setDate(new Date().toISOString().slice(0, 10)); // reset again after submit
  };
