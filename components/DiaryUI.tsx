...
const DiaryUI: React.FC<DiaryProps> = ({ entries, onDeleteEntry }) => {
  return (
        ...
        <div className="space-y-6">
          {entries.length === 0 ? (
            <div className="text-center py-10">
              <p className="text-slate-400 dark:text-slate-500 font-handwriting text-2xl">
                Your diary is empty.
              </p>
              <p className="text-slate-400 dark:text-slate-500 mt-2">
                Add a study session above to begin!
              </p>
            </div>
          ) : (
            entries.map((entry) => (
              <div
                key={entry.id}
                className="flex items-start gap-4 pl-8 relative group"
              >
                  ...
                  <div className="flex justify-between items-start">
                    <div>
                      ...
                      <p className="text-sm text-gray-600 dark:text-gray-400">
                        {formatDateShort(entry.date)} • {entry.timeSlot} •{" "}
                        {entry.hours}h
                      </p>
                    </div>
